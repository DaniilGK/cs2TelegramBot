  ...
  return (
    <div className="flex flex-col px-4 pt-4 pb-4 gap-4 overflow-y-auto animate-fade-in">
      ...
      >
        ...
        <span className="font-display text-base font-semibold text-text-primary tracking-wide">Назад</span>
      </button>

      <div>
        <h1 className="font-display text-2xl font-semibold text-text-primary tracking-wide">Edit Profile</h1>
        <p className="text-xs text-text-secondary font-body">Manage your public RushSkins profile.</p>
      </div>
      ...
      <div className="flex justify-center">
        <div className="relative w-20 h-20 rounded-full bg-accent-orange flex items-center justify-center overflow-hidden shrink-0">
          ...
          <button
            ...
            className="absolute bottom-0 left-0 right-0 h-7 bg-black/50 rounded-b-full text-[9px] text-white font-body flex items-center justify-center"
          >
            Изменить
          </button>
        </div>
      ...
      <div className="bg-bg-raised border border-border rounded-2xl overflow-hidden">
        ...
        >
          <span className="font-display text-base font-semibold text-text-primary tracking-wide">Никнейм</span>
          {isEditingUsername ? (
            <input
              value={username}
              maxLength={32}
            ...
            <span className="text-sm text-text-secondary font-body">{user.username || 'Player'}</span>
          )}
        </div>
        <button
          onClick={() => {
        ...
        >
          <span className="font-display text-base font-semibold text-text-primary tracking-wide">BIO</span>
          <span className="text-sm text-text-secondary font-body truncate">{bio || 'Добавить'}</span>
        </button>
      </div>

      <div className="bg-bg-raised border border-border rounded-2xl p-4 flex flex-col gap-4">
        <p className="text-xs text-text-muted font-body uppercase tracking-wider">НАСТРОЙКИ ПРИВАТНОСТИ</p>
      ...
      {isBioOpen && (
        <div className="fixed inset-0 z-[70] bg-bg-base/95 px-4 pt-4 pb-4 flex flex-col gap-4 animate-fade-in">
          <div className="flex items-center justify-between">
            <h2 className="font-display text-2xl font-semibold text-text-primary tracking-wide">BIO</h2>
            <button
...
function PrivacyToggle({
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      >
        <span className="text-xs text-text-secondary font-body">{option?.label}</span>
