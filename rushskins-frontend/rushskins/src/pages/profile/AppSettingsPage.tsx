...
export function AppSettingsPage() {
  ...
  return (
    <div className="flex flex-col px-4 pt-4 pb-4 gap-4 overflow-y-auto animate-fade-in">
      ...
      <div>
        <h1 className="font-display text-2xl font-semibold text-text-primary tracking-wide">App Settings</h1>
        <p className="text-xs text-text-secondary font-body">Interface preferences and app info.</p>
      </div>

      <div className="bg-bg-raised border border-border rounded-2xl p-4 flex flex-col gap-3">
        <p className="text-xs text-text-muted font-body uppercase tracking-wider">ИНТЕРФЕЙС</p>
        ...
        {theme === 'custom' && (
          <div className="bg-bg-overlay border border-border rounded-xl px-3 py-3 flex flex-col gap-3">
            <span className="text-sm text-text-primary font-body">Accent color</span>
      ...
      <div className="bg-bg-raised border border-border rounded-2xl p-4 flex flex-col gap-4">
        <p className="text-xs text-text-muted font-body uppercase tracking-wider">ПРОЧЕЕ</p>
        <div className="flex items-center justify-between">
          <span className="text-sm text-text-primary font-body">Язык</span>
          <div className="flex bg-bg-overlay border border-border rounded-xl p-1">
            {(['RU', 'EN'] as const).map(option => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-display font-semibold transition-colors ${language === option ? 'bg-accent-orange text-white' : 'text-text-secondary'}`}
              >
                {option}
              </button>
            ))}
        ...
        <div className="flex items-center justify-between">
          <span className="text-sm text-text-primary font-body">Версия приложения</span>
          <span className="text-sm text-text-secondary font-mono">v1.0.0</span>
        </div>
      </div>
    </div>
  )
